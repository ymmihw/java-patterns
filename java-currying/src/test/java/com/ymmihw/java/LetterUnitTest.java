...
import static com.ymmihw.java.Letter.LETTER_CREATOR;
import static com.ymmihw.java.Letter.SIMPLE_CURRIED_LETTER_CREATOR;
import static com.ymmihw.java.Letter.SIMPLE_LETTER_CREATOR;
...
public class LetterUnitTest {
  ...
  private static final Letter LETTER =
      new Letter(RETURNING_ADDRESS, INSIDE_ADDRESS, DATE_OF_LETTER, SALUTATION, BODY, CLOSING);
  ...
  public void whenStaticCurriedFunctionIsCalled_thenaLetterIsCreated() {
    assertEquals(LETTER, LETTER_CREATOR.apply(RETURNING_ADDRESS).apply(CLOSING)
        .apply(DATE_OF_LETTER).apply(INSIDE_ADDRESS).apply(SALUTATION).apply(BODY));
  }
  ...
  public void whenStaticBuilderIsCalled_thenaLetterIsCreated() {
    assertEquals(LETTER,
        Letter.builder().withReturnAddress(RETURNING_ADDRESS).withClosing(CLOSING)
            .withDateOfLetter(DATE_OF_LETTER).withInsideAddress(INSIDE_ADDRESS)
            .withSalutation(SALUTATION).withBody(BODY));
  }
}
